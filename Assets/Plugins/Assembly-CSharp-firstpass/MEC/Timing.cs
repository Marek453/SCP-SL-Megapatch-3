...
namespace MEC
{
	public class Timing : MonoBehaviour
	{
		...
		{
			return (coroutine != null) ? Instance.RunCoroutineInternal(coroutine, Segment.FixedUpdate, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, null, new CoroutineHandle(Instance._instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? Instance.RunCoroutineInternal(coroutine, Segment.FixedUpdate, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, tag, new CoroutineHandle(Instance._instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? Instance.RunCoroutineInternal(coroutine, segment, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, null, new CoroutineHandle(Instance._instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? Instance.RunCoroutineInternal(coroutine, segment, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, tag, new CoroutineHandle(Instance._instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? RunCoroutineInternal(coroutine, Segment.Update, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, null, new CoroutineHandle(_instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? RunCoroutineInternal(coroutine, Segment.Update, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, tag, new CoroutineHandle(_instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? RunCoroutineInternal(coroutine, segment, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, null, new CoroutineHandle(_instanceID), true) : default(CoroutineHandle);
		}
		...
		{
			return (coroutine != null) ? RunCoroutineInternal(coroutine, segment, (!(gameObj == null)) ? new int?(gameObj.GetInstanceID()) : null, tag, new CoroutineHandle(_instanceID), true) : default(CoroutineHandle);
		}
			...
			case Segment.Update:
				...
				while (prewarm)
				{
					...
					else if (UpdateProcesses[processIndex2.i] != null && float.IsNaN(UpdateProcesses[processIndex2.i].Current))
					{
						...
						prewarm = !UpdatePaused[processIndex2.i];
					}
					else
					{
						prewarm = false;
			...
			case Segment.FixedUpdate:
				...
				while (prewarm)
				{
					...
					else if (FixedUpdateProcesses[processIndex2.i] != null && float.IsNaN(FixedUpdateProcesses[processIndex2.i].Current))
					{
						...
						prewarm = !FixedUpdatePaused[processIndex2.i];
					}
					else
					{
						prewarm = false;
			...
			case Segment.LateUpdate:
				...
				while (prewarm)
				{
					...
					else if (LateUpdateProcesses[processIndex2.i] != null && float.IsNaN(LateUpdateProcesses[processIndex2.i].Current))
					{
						...
						prewarm = !LateUpdatePaused[processIndex2.i];
					}
					else
					{
						prewarm = false;
			...
			case Segment.SlowUpdate:
				...
				while (prewarm)
				{
					...
					else if (SlowUpdateProcesses[processIndex2.i] != null && float.IsNaN(SlowUpdateProcesses[processIndex2.i].Current))
					{
						...
						prewarm = !SlowUpdatePaused[processIndex2.i];
					}
					else
					{
						prewarm = false;
			...
			case Segment.RealtimeUpdate:
				...
				while (prewarm)
				{
					...
					else if (RealtimeUpdateProcesses[processIndex2.i] != null && float.IsNaN(RealtimeUpdateProcesses[processIndex2.i].Current))
					{
						...
						prewarm = !RealtimeUpdatePaused[processIndex2.i];
					}
					else
					{
						prewarm = false;
		...
		{
			Timing instance = GetInstance(handle.Key);
			return (!(instance != null) || !instance._handleToIndex.ContainsKey(handle) || !instance._processLayers.ContainsKey(handle)) ? null : new int?(instance._processLayers[handle]);
		}
		...
		public static bool SetSegment(CoroutineHandle handle, Segment newSegment)
		{
			...
			instance.RunCoroutineInternal(instance.CoindexExtract(instance._handleToIndex[handle]), newSegment, (!instance._processLayers.ContainsKey(handle)) ? null : new int?(instance._processLayers[handle]), (!instance._processTags.ContainsKey(handle)) ? null : instance._processTags[handle], handle, false);
			return true;
		}
		...
		{
			Timing instance = GetInstance(handle.Key);
			instance.RunCoroutineInternal(coptr, _tmpSegment, (!instance._processLayers.ContainsKey(handle)) ? null : new int?(instance._processLayers[handle]), (!instance._processTags.ContainsKey(handle)) ? null : instance._processTags[handle], handle, false);
			return null;
		}
		...
		{
			Timing instance = GetInstance(handle.Key);
			instance.RunCoroutineInternal(coptr, _tmpSegment, (!instance._processLayers.ContainsKey(handle)) ? null : new int?(instance._processLayers[handle]), _tmpRef as string, handle, false);
			return null;
		}
