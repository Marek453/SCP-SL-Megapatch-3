		...
		foreach (GameObject item2 in list2)
		{
			...
			if (component.curClass >= 0 && component.klasy[component.curClass].fullName.Contains("939"))
			{
				...
				for (int i = 0; i < list3.Count; i++)
				{
					...
					if (list3[i].position.y < 800f && component2.klasy[component2.curClass].team != 0 && component2.klasy[component2.curClass].team != Team.RIP && !list2[i].GetComponent<Scp939_VisionController>().CanSee(component.GetComponent<Scp939PlayerScript>()))
					{
			...
			foreach (GameObject gameObject in players)
			{
				...
				for (int j = 0; j < array.Length; j++)
				{
					...
					if (!component.isLocalPlayer)
					{
						...
						if (Vector3.Distance(gameObject.transform.position, playerPositionData.position) < 10f && myCCM.curClass != -1 && (component2.curClass != 0 || !myCCM.IsHuman()))
						{
