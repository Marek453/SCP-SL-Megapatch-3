...
public class Scp049_2PlayerScript : NetworkBehaviour
{
	...
	private IEnumerator _UpdateInput()
	{
		while (true)
		{
	...
	{
		if (Vector3.Distance(GetComponent<PlyMovementSync>().position, ply.transform.position) <= distance * 1.5f && GetComponent<CharacterClassManager>().curClass == 10)
		{
			Vector3 position = ply.transform.position;
			CmdPlaySound();
			GetComponent<PlayerStats>().HurtPlayer(new PlayerStats.HitInfo(damage, id, "SCP:0492", GetComponent<QueryProcessor>().PlayerId), ply);
			if (ply.GetComponent<CharacterClassManager>().curClass == 2)
			{
				GetComponent<CharacterClassManager>().CallRpcPlaceBlood(position, 0, 1.3f);
			}
			else
			{
				GetComponent<CharacterClassManager>().CallRpcPlaceBlood(position, 0, 0.5f);
			}
		}
	}
