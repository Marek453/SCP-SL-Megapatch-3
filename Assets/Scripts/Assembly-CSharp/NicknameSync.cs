...
public class NicknameSync : NetworkBehaviour
{
	...
	public string NetworkmyNick
	{
		...
		set
		{
			ref string fieldValue = ref myNick;
	...
	private void Update()
	{
		...
		if (component.curClass != 2 && Physics.Raycast(new Ray(spectCam.position, spectCam.forward), out hitInfo, viewRange, raycastMask) && !FlashEffect.isBlind)
		{
			...
			if (component2 != null && !component2.isLocalPlayer)
			{
				...
				flag = true;
				if (component3.curClass != -1 && !TutorialManager.status)
				{
					n_text.color = component3.klasy[component3.curClass].classColor;
					n_text.text = component2.role.GetColoredRoleString() + "\n";
				...
				try
				{
					if (component3.curClass >= 0 && component3.klasy[component3.curClass].team == Team.MTF && component.klasy[component.curClass].team == Team.MTF)
					{
						...
						if (component.curClass == 4 || component.curClass == 11)
						{
							num = 200;
						}
						else if (component.curClass == 13)
						{
							num = 100;
						}
						else if (component.curClass == 12)
						{
							num = 300;
						}
						Text text2 = n_text;
	...
	private void CmdSetNick(string n)
	{
		...
		else
		{
			...
			if (n == null)
			{
				...
				PlayerManager.localPlayer.GetComponent<BanPlayer>().BanUser(base.gameObject, 26297460, string.Empty);
				SetNick("Null Name");
				return;
			}
