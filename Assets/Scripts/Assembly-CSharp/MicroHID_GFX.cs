...
public class MicroHID_GFX : NetworkBehaviour
{
	...
	private void Update()
	{
		if (base.isLocalPlayer && Input.GetKeyDown(NewInput.GetKey("Shoot")) && GetComponent<Inventory>().curItem == 16 && !onFire && Inventory.inventoryCooldown <= 0f && GetComponent<Inventory>().items[GetComponent<Inventory>().GetItemIndex()].durability > 0f)
		{
			onFire = true;
			CallCmdUse();
	...
	{
		if (GetComponent<Inventory>().curItem == 16 && Vector3.Distance(GetComponent<PlyMovementSync>().position, ply.transform.position) < range && GetComponent<WeaponManager>().GetShootPermission(ply.GetComponent<CharacterClassManager>()) && GetComponent<PlayerStats>().HurtPlayer(new PlayerStats.HitInfo(Random.Range(200, 300), string.Empty, "TESLA", GetComponent<QueryProcessor>().PlayerId), ply) && GetComponent<CharacterClassManager>().klasy[ply.GetComponent<CharacterClassManager>().curClass].team == Team.SCP)
		{
			AchievementManager.Achieve("zap");
		}
	}
	...
	private void GlowLight(int id)
	{
		...
		StartCoroutine(_SetLightState(targetIntensity, progress[id], (id != 5) ? 2f : 50f));
	}

	private IEnumerator _SetLightState(float targetIntensity, Light light, float speed)
	{
