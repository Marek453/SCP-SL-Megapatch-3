...
public class Scp939PlayerScript : NetworkBehaviour
{
	...
	private void CheckForInput()
	{
		if (base.isLocalPlayer)
		{
			...
			bool flag = Input.GetKey(NewInput.GetKey("939's speech")) && iAm939;
			if (prevuhc != flag)
			{
				prevuhc = flag;
	...
	{
		NetworkusingHumanChat = newValue;
	}
	...
	{
		if (Vector3.Distance(attacker.transform.position, base.transform.position) < attackDistance * 1.2f && cooldown <= 0f)
		{
			cooldown = 1f;
			GetComponent<PlayerStats>().HurtPlayer(new PlayerStats.HitInfo(Random.Range(50, 80), "SCP:939", "SCP:939", GetComponent<QueryProcessor>().PlayerId), attacker);
			CallRpcShoot();
		}
	}
