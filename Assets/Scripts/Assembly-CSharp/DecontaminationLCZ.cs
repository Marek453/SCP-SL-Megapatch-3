...
public class DecontaminationLCZ : NetworkBehaviour
{
			...
			foreach (GameObject gameObject in array2)
			{
				if (gameObject != null)
				{
					...
					if (y < 100f && y > -100f)
					{
						...
						ps.HurtPlayer(new PlayerStats.HitInfo(players.Length * Mathf.CeilToInt(component.health / 10) + 10, "DECONT", "DECONT", 0), gameObject);
					}
				}
			}
		}
	...
	{
		if (!NetworkServer.active || curAnm >= announcements.Count || alphaController.inProgress || !ccm.roundStarted)
		{
			return;
		}
		time += Time.deltaTime;
		if (time / 60f > announcements[curAnm].startTime && mtfrespawn.respawnCooldown <= 0f)
		{
			CallRpcPlayAnnouncement(curAnm, announcements[curAnm].global);
			AlphaWarheadController alphaWarheadController = alphaController;
			alphaWarheadController.NetworktimeToDetonation = alphaWarheadController.timeToDetonation + announcements[curAnm].clip.length;
			if (announcements[curAnm].openDoors)
			{
				Invoke("CallOpenDoors", 10f);
			}
			if (announcements[curAnm].global)
			{
				Timing.RunCoroutine(_KillPlayersInLCZ(), Segment.FixedUpdate);
			}
			curAnm++;
		}
	}
