...
public class RoundSummary : NetworkBehaviour
{
	...
	private void Start()
	{
		...
		{
			roundTime = 0;
            StartCoroutine(_ProcessServerSideCode());
			kills_by_scp = 0;
			escaped_ds = 0;
	...
	private IEnumerator _ProcessServerSideCode()
	{
		while (true)
		{
			if (RoundInProgress() && PlayerManager.singleton.players.Length >= 2)
			{
				...
				if (roundEnded)
				{
					...
					if (_anomalies > 0 && _facilityForces == 0 && newList.class_ds == 0 && _d_escape_percentage < 0.15f && (_s_escape_percentage < 0.15f || classlistStart.scientists == 0))
					{
						leadingTeam = LeadingTeam.Anomalies;
					}
					else if (_facilityForces > 0 && _anomalies == 0 && _chaosInsurgency == 0 && _d_escape_percentage < 0.15f && _s_escape_percentage != 0f)
					{
						leadingTeam = LeadingTeam.FacilityForces;
					}
					else if (_chaosInsurgency > 0 && _facilityForces == 0 && _d_escape_percentage != 0f)
					{
						leadingTeam = LeadingTeam.ChaosInsurgency;
					}
					GameConsole.Console.singleton.AddLog("Round finished! Anomalies: " + _anomalies + " | Chaos: " + _chaosInsurgency + " | Facility Forces: " + _facilityForces + " | D escaped percentage: " + Mathf.Round(_d_escape_percentage) * 100f + " | S escaped percentage: : " + Mathf.Round(_s_escape_percentage) * 100f, Color.gray);
					...
					yield return new WaitForSeconds(timeToRoundRestart - 1);
					CallRpcDimScreen();
	...
	{
		StartCoroutine(_ShowRoundSummary(list_start, list_finish, leadingTeam, e_ds, e_sc, scp_kills, round_cd, changed_into_zombies));
	}

	private IEnumerator _ShowRoundSummary(SumInfo_ClassList list_start, SumInfo_ClassList list_finish, LeadingTeam leadingTeam, int e_ds, int e_sc, int scp_kills, int round_cd, int changedIntoZombies)
	{
		...
		ui_root.gameObject.SetActive(value: true);
	...
	private IEnumerator _FadeScreenOut()
	{
		float _s_time = 0f;
		while (_s_time < 1f)
		{
	...
	{
		return PlayerManager.localPlayer != null && PlayerManager.localPlayer.GetComponent<CharacterClassManager>().roundStarted && !RoundSummary.singleton.roundEnded && AlphaWarheadController.host != null;
	}
