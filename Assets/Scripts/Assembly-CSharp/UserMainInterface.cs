...
public class UserMainInterface : MonoBehaviour
{
	...
	public void SetHP(int _hp, int _maxhp)
	{
		...
		textHP.text = Mathf.Clamp(Mathf.Round(sliderHP.value / num * 100f), 1f, 100f) + "%";
		sliderHP.maxValue = num;
	}

	private void Update()
	{
		if(Input.GetKeyDown(KeyCode.P) && GetComponent<Canvas>().enabled == true)
		{
		GetComponent<Canvas>().enabled = false;
		}
		else if (Input.GetKeyDown(KeyCode.P) && GetComponent<Canvas>().enabled == false)
		{
