	...
	private void EscapeFromFacility()
	{
		if (!escaped)
		{
			if (ccm.klasy[ccm.curClass].team == Team.RSC)
			{
				escaped = true;
				ccm.RegisterEscape();
				Timing.RunCoroutine(_EscapeAnim(TranslationReader.Get("Facility", 29)), Segment.Update);
			...
			if (ccm.klasy[ccm.curClass].team == Team.CDP)
			{
				escaped = true;
				ccm.RegisterEscape();
				Timing.RunCoroutine(_EscapeAnim(TranslationReader.Get("Facility", 30)), Segment.Update);
