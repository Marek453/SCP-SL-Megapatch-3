...
public class SoundtrackManager : MonoBehaviour
{
	...
	public class Track
	{
		...
		public void Update()
		{
			...
			source.volume += 0.02f * ((!playing) ? (-1f / exitFadeDuration) : (1f / enterFadeDuration)) * maxVolume;
					...
					try
					{
						...
						if (Physics.Raycast(new Ray(player.transform.position, (item.transform.position - _camera.position).normalized), out hitInfo, 20f, mask))
						{
							...
							if (root.tag == "Player")
							{
								...
								if (ccm.klasy[Mathf.Clamp(curClass, 0, ccm.klasy.Length - 1)].team != 0)
								{
									foundSomeone = true;
								}
							}
