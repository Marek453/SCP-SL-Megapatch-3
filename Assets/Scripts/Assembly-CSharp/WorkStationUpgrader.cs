...
public class WorkStationUpgrader : NetworkBehaviour
{
	...
	public void ChangeSlot(Button button)
	{
		...
		slotID = result;
		curMod = manager.weapons[curWeapon].GetMod((ModPrefab.ModType)slotID);
		RefreshModSelector();
	}
	...
	private void RefreshModSelector()
	{
		...
		manager.weapons[curWeapon].ChangeMod((ModPrefab.ModType)slotID, curMod, true, PlayerManager.localPlayer.GetComponent<WeaponManager>().flashlightEnabled);
	}
