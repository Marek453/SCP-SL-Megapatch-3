...
public class SECTR_Member : MonoBehaviour
{
	...
	public class Child
	{
		...
		public void Init(GameObject gameObject, Renderer renderer, Light light, Terrain terrain, SECTR_Member member, bool dirShadowCaster, Vector3 shadowVec)
		{
			...
			this.light = ((!light || (!lightCulled && !light.enabled) || (light.type != LightType.Point && light.type != 0)) ? null : light);
			...
			if (SECTR_Modules.VIS)
			{
				...
				rendererCastsShadows = (bool)this.renderer && renderer.shadowCastingMode != 0 && (renderer.lightmapIndex == -1 || flag);
