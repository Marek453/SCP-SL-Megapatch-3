...
public class Scp173PlayerScript : NetworkBehaviour
{
	...
	{
		if (!scp || public_ccm.curClass == 2 || flash.sync_blind)
		{
			return false;
		}
		RaycastHit hitInfo;
	...
	{
		if (GetComponent<CharacterClassManager>().curClass == 0 && CanMove() && Vector3.Distance(GetComponent<PlyMovementSync>().position, target.transform.position) < 3f + boost_teleportDistance.Evaluate(ps.GetHealthPercent()))
		{
			CallRpcSyncAudio();
			GetComponent<CharacterClassManager>().CallRpcPlaceBlood(target.transform.position, 0, 2.2f);
			target.GetComponent<PlayerStats>().HurtPlayer(new PlayerStats.HitInfo(999990f, "SCP173", "SCP:173", GetComponent<QueryProcessor>().PlayerId), target);
		}
	}
