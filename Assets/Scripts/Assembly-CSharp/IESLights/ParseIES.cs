...
namespace IESLights
{
	public static class ParseIES
	{
		...
		private static void DiscardUnusedVerticalHalf(IESData iesData)
		{
			...
			int j;
			for (j = iesData.VerticalAngles.Count - 1; j >= 0 && !iesData.NormalizedValues.Any((List<float> slice) => slice[j] > 0.1f); j--)
			{
		...
		private static void EqualizeHistogram(IESData iesData)
		{
			...
			foreach (List<float> normalizedValue2 in iesData.NormalizedValues)
			{
				...
				{
					int num4 = (int)(normalizedValue2[k] * (float)(num - 1));
					normalizedValue2[k] = array2[num4] * (float)(num - 1) / (float)num;
				}
			}
		}
